...
main:	
	...
	jal  	generate_enemy		# call generate_enemy
	
valid_coordinates:
	jal 	check_coalition		# Check collision after grid is printed
	jal 	update_enemy		# update enemy
...
not_reward:
	bne 	$t1,	$s6,	not_enemy	# if $t1 != $s6 --> goto not_enemy
	bne 	$t3,	$s7,	not_enemy	# if $t1 != $s7 --> goto not_enemy
	j 	print_enemy	# jump to print_enemy if no conditions are met above

not_enemy:
	j 	print_space   # jump to print_space if no elements are in the row
